...
void write_to_file(const rsf_results& results,const double coupling)
{
    ...
    for(auto& k_pv:n_kp.second)
    {
        ostringstream ssh;
        ostringstream ssl;
        ssh<<"RSF_n="<<n_kp.first<<"_k="<<k_pv.first<<"_degh_c="<<coupling;
        ssl<<"RSF_n="<<n_kp.first<<"_k="<<k_pv.first<<"_degl_c="<<coupling;
        ...
        {
            fh<<pv.first<<"\t"<<pv.second.deg_h<<endl;
            fl<<pv.first<<"\t"<<pv.second.deg_l<<endl;
        }
    }
}
	...
	for(auto c : cRange)
	{
		...
		for(auto& dp:data)
		{
			...
			for(int perturbCount : perturbCountRange)
			{
				...
				r.deg_h+=analyser.BS_one_config(c,dp,perturbNodes);

				perturbNodes = analyser.lowest_degree(dp.nbr,perturbCount);
				r.deg_l +=analyser.BS_one_config(c,dp,perturbNodes);
		...
		{
			cv.second.deg_h/=cv.second.count;
			cv.second.deg_l/=cv.second.count;
		}
